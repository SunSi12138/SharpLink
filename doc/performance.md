...
本文只发布 `1.0.0-rc3` 精确提交上的最终可复现实测。0.x 开发期的逐版本局部 A/B、临时 runner 结果和优化日志不属于稳定性能承诺，已从用户文档移除；代码历史仍保留在 Git 和 CHANGELOG。

## 当前状态

`1.0.0-rc3` 的代码、公开 API、协议和包表面已进入本地冻结；最终 QPS、吞吐和延迟数字仍必须在该精确 RC 提交完成下述矩阵后发布。测试未完成前不以候选前的开发机结果或短时 smoke 填充本节。
...
原始 JSON、BenchmarkDotNet 报告和环境快照写入 `artifacts/performance/rc3/`，不提交仓库；本文只保存汇总表、命令和解释。
...
SHARPLINK_MATRIX_OUTPUT="$PWD/artifacts/performance/rc3/matrix" \
