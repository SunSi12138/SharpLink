...
测试环境中普通 managed construction 的 padding 曾观测为零，但这不是 C#/.NET 表示契约，也不能把 `new T()`、`default` 或“已赋值所有逻辑字段”当作 padding sanitizer。机密边界上的此类 payload 应显式清理完整 representation，或绑定不使用 SharpLink raw-blit 表示的自定义 Codec/Adapter。完整威胁模型、复现、性能数据与 2.x 产品决策见 [UnsafeBlit padding 安全评估](unsafe-blit-padding-security.md)。
