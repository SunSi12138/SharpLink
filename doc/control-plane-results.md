...
每个 endpoint 的本地 inventory 是自己的 source of truth。duplex stream 的首个元素应是当前完整 snapshot，后续元素携带新的完整 revision；掉线或重连时重新建流并再次发送当前完整 snapshot，本地 reconciler 只根据收到的 peer 声明决定是否更新自己。
