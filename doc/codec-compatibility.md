...
The `auto-layout-release-scoped` fixtures are deliberately evidence-only on Browser <-> hosted-desktop edges. `LayoutKind.Auto` and framework-owned nested value layouts are runtime implementation details, and the current Browser lane is Mono/wasm32 while the hosted desktop lanes are CoreCLR/64-bit. The probe still records size, byte, decode, logical-value, and classification failures for those rows, and validates that each non-blocking row has a recognized layout-mismatch shape, but those unsupported ABI mismatches do not fail the release gate. Browser self-roundtrip, all desktop <-> desktop rows, fixed/sequential/explicit-layout fixtures outside that category, native-width policy checks, fixture completeness, hashes, and every other codec failure remain blocking under their existing contracts.
