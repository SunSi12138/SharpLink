...
This distinction is already useful evidence: historical portable-runtime evidence, including earlier Android/iOS Mono runs, observed a `DateTimeOffsetRaw` representation difference relative to another runtime while the logical fixture definition was the same. That historical observation remains representation evidence only; current-head compatibility claims are tied to the CoreCLR mobile lanes that the active workflow actually executes. When framework semantic fixtures are decoded in the desktop matrix, temporal comparers include observable `DateTime.Kind` and `DateTimeOffset.Offset` state rather than relying on the framework's looser default equality semantics.
