...
A synchronization stream should begin by sending the endpoint's current complete inventory. Later stream items carry newer complete revisions.

Every inventory also carries an opaque `EndpointEpoch`. `Revision` is comparable only while `EndpointEpoch` is unchanged. An endpoint **must** choose a new epoch whenever its revision sequence can reset or declaration authority changes, including a normal process restart unless the application durably preserves the same ordering sequence. When a receiver observes a different epoch, it must accept that complete inventory as a new ordering domain even if its numeric revision is lower than the last revision seen from the old epoch.

After disconnect, reconnect, notification loss, process restart, or reconnecting to another endpoint instance, the Client opens a new synchronization stream and sends its current complete inventory again; the Server likewise returns its current complete inventory again. `GetInventoryAsync` follows the same rule: historical revisions from another epoch must never be used to reject a response as stale.

This avoids correctness depending on replaying an event log, assuming revisions survive process replacement, or requiring both sides to switch at the same instant.

## Inventory and compatibility metadata

Each inventory has an endpoint epoch, a revision that is monotonic within that epoch, and zero or more generation snapshots. Revisions from different epochs have no ordering relationship. A snapshot carries capability/generation identity, wire identity, generated ABI identity, artifact identity/reference, compatibility metadata, local lifecycle state, and a diagnostic code.
