...
`SharpLink.Abstractions` 是跨子系统的稳定公共契约层；`SharpLink.Sdk` 是契约项目的包入口并以 analyzer-only 方式携带 Generator；`SharpLink.GenerationControl` 提供可选的跨端 generation 控制契约；`SharpLink.Hosting` 提供 Generic Host 集成；`SharpLink.Serializer.SharpPack` 提供序列化 Adapter 集成。
