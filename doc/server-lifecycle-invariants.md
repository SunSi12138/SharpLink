...
- `ServerStopOwnershipCharacterizationTests.FirstRunCancellationShouldOwnZeroGraceSharedStopWithoutExplicitStop` verifies cancellation of the first `RunAsync` token alone drives normal zero-grace shutdown while active-call ownership may remain, and that a later explicit stop reuses that already-established cleanup rather than applying a new grace period.
