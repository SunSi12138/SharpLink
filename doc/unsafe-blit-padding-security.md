...
- when such a value crosses a confidentiality boundary, the supported reliable escape path is to bind that payload type to a field-wise/custom Codec or Adapter that does not raw-copy the managed representation;
- representation sanitization is only a valid mitigation when the sanitized storage is the **exact representation subsequently blitted**, with no intervening struct copy whose padding contents are relied upon; caller-side sanitization of a value that may later be copied by generated/proxy code is not documented as a reliable confidentiality control;
...
Cost/complexity: the benchmark models one representative **cached-known-padding** implementation: it performs the same raw write and then iterates cached padding ranges to clear them. Generic production use would additionally need a runtime/AOT-safe way to derive and validate managed-layout padding across nested, explicit, native-width, and runtime-specific layouts.

The benchmark deliberately excludes padding-map discovery cost, but it is **not a strict theoretical lower bound**. This implementation still pays for cached range access, iteration, slicing, and clearing; a source-generated or type-specialized canonicalizer could emit fixed stores and be cheaper. The measurements therefore characterize this concrete cached-range mitigation shape rather than a universal minimum cost.
...
Security effect: removes the selected payload type from the SharpLink `UnsafeBlitCodec<T>` domain when the chosen custom Codec/Adapter uses an appropriate field-wise/non-raw representation.
...
`UnsafeBlitPaddingBenchmarks` compares the current production `UnsafeBlitCodec<T>` hot path with a representative cached-known-padding post-blit clear implementation for `ByteInt32`, `ByteInt64`, `Int64Byte`, and `NestedPadding`.
...
No benchmark variant allocated managed memory. The hosted-runner `ShortRun` configuration has only three measured iterations and some individual confidence intervals are wide, so the figures are directional rather than a release-grade microbenchmark budget. The consistent result across all three environments is that clearing already-known padding with this cached-range implementation is not free; nested/multiple padding ranges are the most expensive case in this sample.

Because the candidate assumes an already-known padding map, these results isolate the cost of this cached-range implementation from padding-map discovery. They are **not** a theoretical minimum: source-generated/type-specialized fixed stores could be cheaper, while a generic production implementation could add layout discovery, validation, caching, NativeAOT/trimming, and maintenance complexity.
...
3. classify unsafe/native/uninitialized source provenance as unsuitable for confidential raw-blit RPC boundaries; the reliable supported escape path is a field-wise/custom Codec/Adapter that leaves the raw-blit representation, while representation sanitization is only valid when the sanitized storage is the exact representation subsequently blitted with no intervening copy whose padding contents are relied upon;
