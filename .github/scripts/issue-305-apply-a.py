...
'''        AdmissionPartitionEntry? partitionEntry = null;
        ...
        var request = CreateRequest(context, _partitions, partitionEntry);''')
...
'''    private AdmissionRequest CreateRequest(
        SharpLinkAdmissionContext context,
        AdmissionPartitionPool? partitionOwner,
        AdmissionPartitionEntry? partitionEntry)
    {
        ...
        return new AdmissionRequest(slots, count, partitionOwner, partitionEntry);
    }''')

replace(
    '    private AdmissionPartitionLease? _partition;',
    '    private AdmissionPartitionPool? _partitionOwner;\n    private AdmissionPartitionEntry? _partitionEntry;')
replace(
...
'''        var partitionEntry = Interlocked.Exchange(ref _partitionEntry, null);
        var partitionOwner = Interlocked.Exchange(ref _partitionOwner, null);
        if (partitionEntry is not null)
            partitionOwner!.Release(partitionEntry);
        owner.OnLeaseDisposed();''')
...
'''internal sealed class AdmissionRequest(
    ...
    int slotCount,
    AdmissionPartitionPool? partitionOwner,
    AdmissionPartitionEntry? partitionEntry) : IDisposable
{
    private AdmissionPartitionPool? _partitionOwner = partitionOwner;
    private AdmissionPartitionEntry? _partitionEntry = partitionEntry;''')
...
'''        Interlocked.Exchange(ref _partition, null)?.Dispose();
    ...
    private static bool HasRetainedSlot''',
'''        var partitionEntry = Interlocked.Exchange(ref _partitionEntry, null);
        var partitionOwner = Interlocked.Exchange(ref _partitionOwner, null);
        if (partitionEntry is not null)
            partitionOwner!.Release(partitionEntry);
...
if 'AdmissionPartitionLease' in text:
    raise SystemExit('candidate A left an AdmissionPartitionLease reference')
...
public sealed class AdmissionPartitionOwnershipTests
{
    ...
    public async Task RequestAndLeaseShouldReleasePartitionEntryExactlyOnce()
    {
        ...
        using var firstRequest = new AdmissionRequest(slots, count, pool, firstEntry);
        ...
        using var secondRequest = new AdmissionRequest(secondSlots, count, pool, secondEntry);
