...
- A Request frame now carries the remaining `TimeBudget` sampled once when the frame is created, and the send pump copies frames verbatim. The pump no longer defers timed writes, re-samples `GetRemaining` per frame, compacts frames whose deadline elapsed while they waited in the local queue, or retains a process-local `EmissionDeadline` on the writer lease or the queued frame. The caller's own deadline stays authoritative end to end: a call that registers a pending entry still fails through its pending deadline, and cancellation still terminates a remote call that already started. Two client-side gaps that the removed compaction used to cover are closed with it: a plain OneWay now races its emission wait against its own sampled deadline, so it cannot block past it, and a Request whose deadline elapses while it is still being serialized is no longer published after the cancel that same deadline emitted. The same work closes the pair of guarantees around them: a plain OneWay whose budget elapses after its Request was published now sends the deadline cancel behind that Request, so the peer stops work whose caller has already given up, and a Request is reported as published only once the send queue accepted it, so a refused frame can no longer produce a cancel for a Request the peer never received. Frame preparation, including a user-provided compression provider, now runs before that publication decision instead of while the decision is being arbitrated, and a plain OneWay re-checks its budget after that preparation, so a Request that expired while the compression provider ran is never published. A shape that owns a pending entry - OneWay with client streams, client streaming and duplex streaming - also waits for the emission of its head Request on the pending call's own producer token, so the deadline, a caller cancellation or a closed connection ends the invocation from a stalled transport write instead of leaving it blocked in the send path. The wire budget is sampled before the request payload and metadata are serialized, so the value the server sees no longer deducts the rest of the serialization, the client send queue, or the network, and it is advisory: it bounds how long the server is willing to keep working, while the caller's deadline remains the one that decides the call, which matches the gRPC model where a deadline is fixed at invocation time. A timed Request also now waits for the configured batch window like any other frame when the caller sets an explicit `RpcSessionFlushOptions.MaxLatency`; previously timed frames bypassed that window, and they remain immediately emitted under the default profiles, where the batch window is zero. The pump still coalesces a batch into one output span at flush time, so dropping the deadline machinery does not cost the transport its segment coalescing.
...
- Internal error-send helpers accept already structured `SharpLinkException`; public Session helpers are removed. Applications customize exception mapping through the Server SPI.
