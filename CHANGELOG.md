...
- Runtime sessions now receive one immutable creation snapshot containing their Client/Server role, real Runtime Context, flush policy, and optional Server exception mapper. Context-derived protocol limits and the sole StreamManager instance are established before the constructor returns.
...
- Client, static/dynamic cluster, and Server connection paths now construct complete `RpcSession` instances before handshake. Runtime Context, role-specific telemetry, StreamManager, and Server mapper state are read-only for the Session lifetime; stream dispatcher codec-provider overloads also require an explicit provider.
