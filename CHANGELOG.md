...
- Server connection shutdown now first signals terminal stream and send-pump state, then cancels and joins the session read loop before completing its `PipeReader`. Rolling restart can neither strand bounded stream dispatch during cleanup nor reclaim a live `ReadOnlySequence` while the frame parser is consuming it, eliminating teardown timeouts and the resulting `ArgumentOutOfRangeException` without hiding malformed protocol frames.
