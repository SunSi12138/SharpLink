...
- Server connection shutdown now cancels and joins the session read loop before completing its `PipeReader`. Rolling restart can no longer reclaim a live `ReadOnlySequence` while the frame parser is consuming it, eliminating the resulting teardown-only `ArgumentOutOfRangeException` without hiding malformed protocol frames.
