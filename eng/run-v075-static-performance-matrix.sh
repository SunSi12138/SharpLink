...
ROOT="${SHARPLINK_V075_MATRIX_ROOT:-$SCRIPT_ROOT}"
TIER="${SHARPLINK_V075_MATRIX_TIER:-smoke}"
RUNTIMES="${SHARPLINK_V075_MATRIX_RUNTIMES:-jit}"
OUTPUT_ROOT="${SHARPLINK_V075_MATRIX_OUTPUT:-$ROOT/artifacts/performance/v0.7.5/static}"
WARMUP="${SHARPLINK_V075_MATRIX_WARMUP:-1}"
DURATION="${SHARPLINK_V075_MATRIX_DURATION:-3}"
...
case "$TIER" in
  ...
  full)
    ...
    ;;
  *)
    echo "Unsupported SHARPLINK_V075_MATRIX_TIER: $TIER" >&2
    exit 2
    ;;
esac
...
run_case() {
  ...
  local output="$6"
  local args=(
    --mode local --transport tcp --static-endpoints "$endpoint_count"
...
echo "0.7.5 static endpoint performance matrix complete: $OUTPUT_ROOT"
