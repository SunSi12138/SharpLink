...
EVIDENCE_ROOT="$TEMP_ROOT/issue252-experiment-b"
BASE_ROOT="$TEMP_ROOT/issue252-b-base"
CAND_ROOT="$TEMP_ROOT/issue252-b-candidate"

rm -rf "$EVIDENCE_ROOT" "$BASE_ROOT" "$CAND_ROOT"
mkdir -p "$EVIDENCE_ROOT"

echo "[issue252-B] preparing paired worktrees"
...
echo "[issue252-B] base_sha=$BASE_SHA"
echo "[issue252-B] candidate_source_sha=$CAND_SOURCE_SHA"

(
  cd "$CAND_ROOT"
  python3 eng/issue252-experiment-b.py
  git diff --check
  git diff --stat
)

echo "[issue252-B] validating candidate like fixed PR CI"
(
  cd "$CAND_ROOT"
  ...
  if ! dotnet test --project test/SharpLink.UnitTests/SharpLink.UnitTests.csproj -c Release --no-build; then
    echo "[issue252-B] first unit-test pass failed; retrying once to separate known unrelated CI flakes"
    dotnet test --project test/SharpLink.UnitTests/SharpLink.UnitTests.csproj -c Release --no-build
  fi
)

echo "[issue252-B] building benchmark baseline"
...
run_variant() {
  ...
  echo "[issue252-B] round=$round variant=$name benchmark=single"
  (
    cd "$root"
  ...
  echo "[issue252-B] round=$round variant=$name benchmark=cross"
  (
    cd "$root"
    dotnet run --project test/SharpLink.Benchmarks/SharpLink.Benchmarks.csproj -c Release --no-build -- \
...
run_variant 1 experiment-b "$CAND_ROOT"
run_variant 2 experiment-b "$CAND_ROOT"
run_variant 2 eager-dev "$BASE_ROOT"
run_variant 3 eager-dev "$BASE_ROOT"
run_variant 3 experiment-b "$CAND_ROOT"
...
for number in (1, 2, 3):
    ...
    cand_single = mean_ns(root / f'round{number}-experiment-b' / 'single.log')
    base_cross = mean_ns(root / f'round{number}-eager-dev' / 'cross.log')
    cand_cross = mean_ns(root / f'round{number}-experiment-b' / 'cross.log')
    rounds.append({
        'round': number,
...
result = {
    'experiment': 'B-segmented-helper-operation-local-location',
    'gate_percent': 3.0,
    'rounds': rounds,
...
print('EXPERIMENT_B_RESULT=' + json.dumps(result, separators=(',', ':')))
if not passed:
    raise SystemExit('Experiment B missed the stable <=3% gate')
PY
