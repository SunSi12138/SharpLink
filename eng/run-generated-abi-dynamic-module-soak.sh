...
replacement_uid="$(jq -r '.tests[] | select(.type.methodName == "OneHundredClientReplacementsShouldLeaveOneReusableRegistration") | .uid' "$OUTPUT/tests.json")"
route_drain_uid="$(jq -r '.tests[] | select(.type.methodName == "MultiClusterReplacementShouldPublishCoordinatorRoutesBeforeOldDrainAndAfterCallerCancellation") | .uid' "$OUTPUT/tests.json")"
rejection_uid="$(jq -r '.tests[] | select(.type.methodName == "RejectedApi4DynamicRegistrationShouldReleaseItsCollectibleContext") | .uid' "$OUTPUT/tests.json")"
framework_unload_uid="$(jq -r '.tests[] | select(.type.methodName == "CollectibleContextShouldUnloadAfterFrameworkReferencesAreReleased") | .uid' "$OUTPUT/tests.json")"
mapfile -t stream_uids < <(jq -r '
  .tests[] |
  select(.type.methodName == "Api4DynamicStreamExitShouldReleaseItsCollectibleContext") |
  .uid' "$OUTPUT/tests.json")
if [[ -z "$replacement_uid" || "$replacement_uid" == "null" ||
      -z "$route_drain_uid" || "$route_drain_uid" == "null" ||
...
printf 'commit=%s\nduration_seconds=%s\nrounds=%s\ntest_processes=%s\nclient_registration_replacements=%s\nroute_publication_before_old_drain=%s\napi4_stream_exits=%s\napi4_stream_exit_modes=%s\nregistration_rejections=%s\nframework_reference_unloads=%s\n' \
  "$(git -C "$ROOT" rev-parse HEAD)" \
  "$elapsed_seconds" \
  "$rounds" \
  "$((rounds * (4 + ${#stream_uids[@]})))" \
  "$((rounds * 100))" \
  "$rounds" \
  "$((rounds * ${#stream_uids[@]}))" \
