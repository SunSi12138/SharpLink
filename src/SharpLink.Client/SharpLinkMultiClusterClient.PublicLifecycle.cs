...
internal sealed partial class SharpLinkMultiClusterClient
{
    ...
    private Task? _runningConnectTask;
    ...
    ValueTask ISharpLinkMultiClusterClient.ConnectAsync(CancellationToken cancellationToken)
    {
        if (LifecycleState != SharpLinkClientLifecycleState.Running)
        ...
        lock (_gate)
        {
            ...
            if (_runningConnectTask is null || _runningConnectTask.IsCompleted)
            {
                _runningConnectTask = ConnectRunningClustersAsync();
                TrackFrameworkTask(
                    _runningConnectTask,
                    "MultiClusterCompatibilityConnect",
                    TaskObservationMode.ExternallyObserved);
            }
            operation = _runningConnectTask;
        }
    ...
    private async Task ConnectRunningClustersAsync()
    {
        var slots = Volatile.Read(ref _snapshot).Slots;
        await Parallel.ForEachAsync(
            slots,
