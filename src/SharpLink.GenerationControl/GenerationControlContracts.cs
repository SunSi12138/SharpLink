...
{
    /// <summary>
    /// Gets or initializes the opaque ordering epoch for this endpoint's revision sequence.
    /// </summary>
    /// <remarks>
    /// Revisions are comparable only when this value is equal. The endpoint must change the epoch whenever
    /// its revision sequence can restart or declaration authority changes. A receiver that observes a different
    /// epoch must accept the complete inventory as a new ordering domain regardless of the numeric revision.
    /// </remarks>
    public string EndpointEpoch { get; init; } = string.Empty;

    /// <summary>Gets or initializes the monotonic revision within <see cref="EndpointEpoch"/>.</summary>
    /// <remarks>Callers must not compare this value across different endpoint epochs.</remarks>
...
{
    /// <summary>Returns the Server endpoint's current complete generation inventory.</summary>
    /// <remarks>
    /// A response is ordered only against inventories with the same endpoint epoch. If the epoch differs from
    /// a previously observed response, callers must accept this complete inventory regardless of revision.
    /// </remarks>
    ...
    /// <remarks>
    /// Each direction uses the inventory epoch as the revision ordering scope. A changed epoch starts a new
    /// ordering domain and the complete inventory must be accepted even when its revision is numerically lower.
    /// </remarks>
