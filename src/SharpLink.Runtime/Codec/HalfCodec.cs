...
internal sealed class HalfCodec : IRpcCodec<Half>
{
    internal static readonly HalfCodec Instance = new();
    private const int Size = 2;

    [MethodImpl(MethodImplOptions.AggressiveInlining)]
    public void Serialize(in Half value, in ArrayBufferWriter<byte> writer)
    {
    ...
    public Half Deserialize(in ReadOnlySequence<byte> buffer)
    {
        ...
        buffer.CopyTo(temp);
        return Unsafe.ReadUnaligned<Half>(ref MemoryMarshal.GetReference(temp));
    }
}

internal sealed class NullableHalfCodec : IRpcCodec<Half?>
{
    internal static readonly NullableHalfCodec Instance = new();
