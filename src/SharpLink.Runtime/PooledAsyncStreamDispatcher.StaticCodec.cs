...
/// <typeparam name="TCodec">The statically known Codec type retained by this dispatcher.</typeparam>
/// <remarks>Dispose the enumerator to release buffered items and return the dispatcher to its pool.</remarks>
internal sealed class PooledAsyncStreamDispatcher<T, TCodec> :
...
{
    private static readonly ConcurrentStack<PooledAsyncStreamDispatcher<T, TCodec>> Pool = [];
    ...
    /// <summary>Rents a dispatcher with the generated codec type preserved in the closed generic.</summary>
    public static PooledAsyncStreamDispatcher<T, TCodec> Rent(
        ...
        bool payloadNullable)
    {
        ...
        dispatcher.Reset(enumerationToken, in codec, payloadNullable);
    ...
    private ValueTask DispatchAcquiredAsync(ReadOnlySequence<byte> payload, int encodedByteCount)
    {
        ...
        try
        {
            ...
            item = _codec.Deserialize(payload);
    ...
    internal long RetainForRegistration()
    {
        if (!TryAcquireDispatch(out var leaseState))
            throw new ObjectDisposedException(
                typeof(PooledAsyncStreamDispatcher<T, TCodec>).FullName,
    ...
    public IAsyncEnumerator<T> GetAsyncEnumerator(CancellationToken cancellationToken = default)
    {
        ...
        _enumerationCancellationRegistration = _enumerationToken.CanBeCanceled
            ? _enumerationToken.UnsafeRegister(static state => ((PooledAsyncStreamDispatcher<T, TCodec>)state!).Signal(), this)
            : default;
        _additionalEnumerationCancellationRegistration = _additionalEnumerationToken.CanBeCanceled
            ? _additionalEnumerationToken.UnsafeRegister(static state => ((PooledAsyncStreamDispatcher<T, TCodec>)state!).Signal(), this)
            : default;
    ...
    internal bool HasRetainedReferencesForTests
    {
        get
        {
            if (_error is not null || Volatile.Read(ref _dispatchState) is not null ||
