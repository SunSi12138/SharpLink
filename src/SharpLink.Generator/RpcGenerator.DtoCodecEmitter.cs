...
public partial class RpcGenerator
{
        ...
        IReadOnlyDictionary<string, string> concreteCodecTypes)
    {
        ...
        sb.AppendLine("{");
        sb.AppendLine("    private readonly Core _core;");
        sb.AppendLine();
        ...
        sb.AppendLine("        _core = new Core(provider);");
        sb.AppendLine("    }");
        sb.AppendLine();
        sb.AppendLine("    internal Core Core => _core;");
        sb.AppendLine("    public bool CanExactSize => _core.CanExactSize;");
        sb.AppendLine($"    public void Serialize(in {model.TypeName} value, IBufferWriter<byte> buffer) => _core.Serialize(in value, buffer);");
        sb.AppendLine($"    public {model.TypeName} Deserialize(in ReadOnlySequence<byte> buffer) => _core.Deserialize(in buffer);");
        sb.AppendLine($"    public bool TryGetEncodedSize(in {model.TypeName} value, out int size) => _core.TryGetEncodedSize(in value, out size);");
        sb.AppendLine($"    public bool TryGetEncodedSize(in {model.TypeName} value, out int size, out IRpcSizedCodecSnapshot? snapshot) => _core.TryGetEncodedSize(in value, out size, out snapshot);");
        sb.AppendLine($"    public void SerializeSized(in {model.TypeName} value, IBufferWriter<byte> buffer, int size, IRpcSizedCodecSnapshot? snapshot) => _core.SerializeSized(in value, buffer, size, snapshot);");
        sb.AppendLine("    public void ReleaseSnapshot(IRpcSizedCodecSnapshot? snapshot) => _core.ReleaseSnapshot(snapshot);");
        sb.AppendLine();
        sb.AppendLine($"    internal readonly struct Core : IRpcCodec<{model.TypeName}>, IRpcSizedCodec<{model.TypeName}>");
        sb.AppendLine("    {");
        sb.AppendLine("        private readonly global::System.Collections.Concurrent.ConcurrentBag<__SizedSnapshot> __snapshotPool;");
        sb.AppendLine();
        ...
        sb.AppendLine();
        sb.AppendLine("        internal Core(IRpcCodecProvider provider)");
        sb.AppendLine("        {");
        sb.AppendLine("            __snapshotPool = new global::System.Collections.Concurrent.ConcurrentBag<__SizedSnapshot>();");
        for (var index = 0; index < complexMembers.Length; index++)
        {
            var member = complexMembers[index];
            sb.AppendLine(
